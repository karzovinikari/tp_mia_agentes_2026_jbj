...
M2 se construye sobre el agente ReAct de M1 sin cambiar la estructura externa (`build_agent`, `register_tool`, `run`). Toda la lógica nueva, estado conversacional, ventana de contexto, salida estructurada, reintentos y tracking de tokens, vive dentro de `MyAgent` (`student_framework/agent.py`). El agente ahora realiza llamadas sucesivas a `run()` sobre la misma instancia continúan la misma conversación, con un historial que se recorta a `max_history_messages` antes de cada llamada al modelo.

## 1. Estrategia de memoria

La estrategia es **sliding window por recencia**: `_history` acumula todos los mensajes de la conversación (turnos de usuario, respuestas del asistente con sus `tool_calls` y resultados `role: "tool"`), y antes de cada llamada al LLM `_window()` devuelve solo los últimos `max_history_messages`. El recorte es una *vista* sobre el historial: el historial completo sigue en memoria, de modo que cambiar de estrategia (summarization, retrieval) no requeriría tocar el resto del bucle.

Se eligió conservar los mensajes más recientes y descartar los más viejos porque en un diálogo el contexto inmediato es el que condiciona la respuesta: el último pedido del usuario, la última salida de una herramienta, etc. El tradeoff es que el agente olvida cosas mencionadas mensajes atrás una vez que salen de la ventana; una estrategia de summarization los preservaría comprimidos, pero agregaría llamadas extra al LLM. 
...
`structured_call(prompt, schema, max_repair_attempts=2)` exige la respuesta a través de la herramienta `final_result`, cuyo esquema se arma con `final_result_tool_schema(schema)` (nombre fijo `mia_agents.FINAL_RESULT_TOOL_NAME`). En cada llamada a `chat` se pasa `tools=[final_result]`: el modelo no recibe ninguna otra herramienta, y la descripción de la tool le indica que debe invocarla para terminar.

El supuesto es `1 + max_repair_attempts` llamadas al LLM. Cada intento puede fallar de tres modos, y cada modo dispara un mensaje de reparación específico:
...
En caso correcto devuelve la instancia Pydantic validada. Si se agotan los intentos, `structured_call` levanta `StructuredOutputError` con el conteo de intentos y el último error; nunca devuelve `None` ni una instancia parcial. Con `max_repair_attempts=2` eso significa exactamente 3 llamadas al LLM antes de fallar.

## 3. Resiliencia y tracking de tokens

Las llamadas al cliente LLM y a las herramientas pasan por el mismo método de reintentos (`_chat_with_retries` / `_call_tool_with_retries`). Un fallo se considera **transitorio** si es `TimeoutError`/`ConnectionError` o si el texto de la excepción contiene marcadores típicos de proveedor (`timeout`, `throttl`, `rate`, `429`, `500`, `502`, `503`, `overload`, `unavailable`, ...). Los transitorios se reintentan hasta `max_retries` veces adicionales con backoff exponencial (`retry_base_delay · 2^intento`); cualquier otro error como bugs, credenciales inválidas, argumentos incorrectos frena en el primer intento, porque reintentarlo solo demoraría el mismo fallo.
...
- Operando no numérico: nombra el parámetro, el valor y el tipo recibidos. Ej: `Error recuperable: el parámetro 'a' debe ser numérico, pero se recibió 'tres' (tipo str). Reintentá pasando un número, por ejemplo 3 o 2.5.` (`bool` se rechaza explícitamente: en Python es subclase de `int`).
- Operador no soportado: lista el conjunto permitido. `Operadores permitidos: +, -, *, /, %`.
- División o módulo por cero: explica la restricción y qué parámetro la viola. Ej: `el divisor 'b' vale 0 y la división por cero no está definida`.

**Lector de archivos** (`file_reader`), ahora acotado al sandbox `student_framework/sandbox/`:

- Ruta vacía, absoluta o con `..`: indica la regla violada y muestra cómo debe verse una ruta válida (`'notas.txt'`, `'datos/info.txt'`).
...
La raíz del sandbox es una constante (`SANDBOX_ROOT`) y no un parámetro de la función: si fuera parámetro aparecería en el esquema y el propio LLM podría elegir otra raíz.
...
## 5. Modos de fallo: dentro y fuera del alcance
...
3. **Presupuesto por tokens:** el límite es por cantidad de mensajes, no por tamaño; un mensaje individual gigante puede exceder el contexto real del modelo aunque la ventana respete N.
4. **Errores de tool señalizados como string:** un `"Error recuperable: ..."` es indistinguible de un contenido legítimo que empiece igual; un canal de error tipado (excepciones controladas o un campo estructurado) sería más robusto.
